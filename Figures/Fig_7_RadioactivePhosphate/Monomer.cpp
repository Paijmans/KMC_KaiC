...
void Monomer::fire_reaction(int reaction_channel)
{
  switch (reaction_channel)
  {
    case 0:  nI = 0; sys->CIATPcons++; //CI-ATP -> CI-ADP + Pi
             if( radnI ){ radnI = 0; sys->radPi++; } //Assume nucl exchange=0.    
    break;
    case 1:  nI = 1; //CI-ADP + ATP -> CI-ATP + ADP
    break;
    case 2:  nII = 0; sys->CIIATPcons++; //CII-ATP -> CII-ADP + Pi
             if( radnII ){ radnII = 0; sys->radPi++; } //Assume nucl exchange=0.
    break;
    case 3:  nII = 1; //CII-ADP + ATP -> CII-ATP + ADP
    break;
    case 4:  nII = 0; T = 1; S = 0; sys->CIIATPcons++; // U <-> T
             if( radnII ){ radnII=0; radT=1; }
    break;
    case 5:  if( nII ){ sys->radPi++; sys->CIIATPcons++; }
             nII = 1; T = 0; S = 0; sys->CIIATPcons--;
             if( radT   ){ radnII=1; radT=0; }
    break;
    case 6:  nII = 0; T = 1; S = 1; sys->CIIATPcons++; // T <-> D
             if( radnII ){ radnII=0; radS=1; }
    break;
    case 7:  if( nII ){ sys->radPi++; sys->CIIATPcons++; }
             nII = 1; T = 1; S = 0; sys->CIIATPcons--;
             if( radS   ){ radnII=1; radS=0; }
    break;
    case 8:  nII = 0; T = 1; S = 1; sys->CIIATPcons++; // S <-> D
             if( radnII ){ radnII=0; radT=1; }
    break;
    case 9:  if( nII ){ sys->radPi++; sys->CIIATPcons++; }
             nII = 1; T = 0; S = 1; sys->CIIATPcons--;
             if( radT   ){ radnII=1; radT=0; }
    break;
    case 10: nII = 0; T = 0; S = 1; sys->CIIATPcons++; // U <-> S
             if( radnII ){ radnII=0; radS=1; }
    break;
    case 11: if( nII ){ sys->radPi++; sys->CIIATPcons++; }
             nII = 1; T = 0; S = 0; sys->CIIATPcons--;
             if( radS   ){ radnII=1; radS=0; }
    break;
    case 12: T = 0; S = 0;
             if( radT   ){ radT=0; sys->radPi++;}
    break;
    case 13: T = 1; S = 0;
             if( radS   ){ radS=0; sys->radPi++;}
    break;        
    case 14: T = 0; S = 1;
             if( radT   ){ radT=0; sys->radPi++;}
    break;
...
{ 
  ...
  // PHOSPHOTRANSFER CHANNELS:
  // CII: U <-> T
  prop_list[4]  = (double) (1-T) * (1-S) * nII * kUT;
  prop_list[5]  = (double) T * (1-S) * (1-nII) * kTU;
  //prop_list[5]  = (double) T * (1-S) * kTU;
  // CII: T <-> D
  prop_list[6]  = (double) T * (1-S) * nII * kTD;
  prop_list[7]  = (double) T * S * (1-nII) * kDT;
  //prop_list[7]  = (double) T * S * kDT;  
  // CII: S <-> D
  prop_list[8]  = (double) (1-T) * S * nII * kSD;
  prop_list[9]  = (double) T * S * (1-nII) * kDS;
  //prop_list[9]  = (double) T * S * kDS;  
  // CII: U <-> S    
  prop_list[10] = (double) (1-T) * (1-S) * nII * kUS;
  prop_list[11] = (double) (1-T) * S * (1-nII) * kSU;
  //prop_list[11] = (double) (1-T) * S * kSU;  


  // SPONTANIOUS DEPHOSPHORYLATION CHANNELS:
  // CII: T -> U
  prop_list[12]  = (double) T * (1-S) *reaction_consts-> kTUs;
  // CII: D -> T
  prop_list[13]  = (double) T * S * reaction_consts->kDTs;  
  // CII: D -> S
  prop_list[14]  = (double) T * S * reaction_consts->kDSs;  
  // CII: S -> U    
  prop_list[15] = (double) (1-T) * S * reaction_consts->kSUs;  
