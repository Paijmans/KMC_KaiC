...
{ 
  ...
  //Read in minima from file.
  
  ifstream infile("ABC_ATP50.minima");
  string line;
  double tminima1, tminima2, temp;
  int idx(0);
  ...
  while (getline(infile, line))
  {
    ...
    if(iss >> tminima2 >> temp)
    {
      sys.treg[idx] = sys.start_frac_reg * (tminima2 - tminima1) + tminima1;
      cerr << idx << " " << tminima1 << " "<< tminima2 << " " << sys.treg[idx] << endl;
      idx++;     
      tminima1 = tminima2;
    }
    else
    {
      cerr << "Wrong format in minima file" << endl;
      exit(1);
    }
  }
  cerr << idx << " minima read from file." << endl;
  ...
  { 
    ...
    {   
      //Ensemble snapshots when all KaiA is sequestered/ released. 0.01/0.05 or 0.3/0.4
      /*
      if(hex_avr.Afree + hex_avr.ACII < 0.01 && !FullKaiASeqisNow)
      {
        cerr << "KaiA low: " << sys.tsim << endl;
        FullKaiASeqisNow=1;
        for(int i(0); i<sys.N_hexamers; i++)
        {
           cout << "Alo:" << hexamers[i].get_index() << "-\t" << hexamers[i].get_hex_pU() 
           << "\t" << hexamers[i].get_hex_pT() << "\t" << hexamers[i].get_hex_pD()
           << "\t" << hexamers[i].get_hex_pS() << "\t" << hexamers[i].get_CIKaiA_bound()
           << "\t" << hexamers[i].get_CIKaiB_bound() << "\t" << sys.tsim << endl;
        }        
      }
      
      if(hex_avr.Afree + hex_avr.ACII > 0.05 && FullKaiASeqisNow)
      {
        FullKaiASeqisNow=0;
        cerr << "KaiA up: " << sys.tsim << endl;
        for(int i(0); i<sys.N_hexamers; i++)
        {
           cout << "Aup:" << hexamers[i].get_index() << "-\t" << hexamers[i].get_hex_pU() 
           << "\t" << hexamers[i].get_hex_pT() << "\t" << hexamers[i].get_hex_pD()
           << "\t" << hexamers[i].get_hex_pS() << "\t" << hexamers[i].get_CIKaiA_bound()
           << "\t" << hexamers[i].get_CIKaiB_bound() << "\t" << sys.tsim << endl;
        }        
  ...
  /* ### For flux histogram output WITH INTERVALS ### 
  temp = 0.;
  for( int nT=0; nT<HIST_RES*6+1; nT++ )
  {
    for( int nS=0; nS<HIST_RES*6+1; nS++ )
    {
      ...
      cout << nT << "\t" << nS << "\t" 
           << (double)sys.hist_pTVSpS[nT][nS]/(sys.interval_reg * (idx-2.0) * sys.N_hexamers) << "\t"
           << (double)sys.rate_pTp1[nT][nS]/(sys.interval_reg * (idx-2.0) * sys.N_hexamers) << "\t"
           << (double)sys.rate_pTm1[nT][nS]/(sys.interval_reg * (idx-2.0) * sys.N_hexamers) << "\t"
           << (double)sys.rate_pSp1[nT][nS]/(sys.interval_reg * (idx-2.0) * sys.N_hexamers) << "\t"
           << (double)sys.rate_pSm1[nT][nS]/(sys.interval_reg * (idx-2.0) * sys.N_hexamers) << endl;
    }
  }
  cerr << "Total acc time: " << temp/sys.N_hexamers << " of " << sys.interval_reg * (idx-2.0) << endl;
  */
  
  /* ### For flux histogram output with NO INTERVALS ### */
  for( int nT=0; nT<HIST_RES*6+1; nT++ )
  {
    for( int nS=0; nS<HIST_RES*6+1; nS++ )
    {
...
HexamerAvr calc_hex_averages( Hexamer *hexamers, SystemVariables *sys, ReactionConstants *reaction_consts )
{
  ...
  {  
    if( hexamers[i].get_active() )
    {
      ...
      //if( hexamers[i].get_CIKaiA_bound() == 0 ) ACIB+=hexamers[i].get_CIKaiB_bound();        
    ...
    {  
      ...
      //if( hexamers[i].get_CIKaiA_bound() == 0 ) ICIB+=hexamers[i].get_CIKaiB_bound(); 
  ...
  /* Radioactive phosphates tracking */
  /*
  int tot_radST( 0. ), tot_radnII( 0. ), tot( 0.);
  for(int i(0); i<sys->N_hexamers; i++)
  {
    for(int j(0); j<6; j++ )
    {
      if( hexamers[i].get_Monomer(j)->radT ) tot_radST++;
      if( hexamers[i].get_Monomer(j)->radS ) tot_radST++;    
      if( hexamers[i].get_Monomer(j)->radnII ) tot_radnII++;    
    }
  }
  tot = tot_radST + tot_radnII + sys->radPi;
  cout << sys->tsim << " " << (double)tot_radST/tot << " " << (double)tot_radnII/tot 
       << " " << (double)sys->radPi/tot << " " << tot << endl;
  */
  ...
  */
  ...
  // Print hexamer states conditioned on pS.
  /*
  for(int i(0); i<sys->N_hexamers; i++)
  { 
    if( hexamers[i].get_CIKaiB_bound() == reaction_consts->nBseq )
  ...
  */
  ...
  /*
  //Print hexamer phosphorylation S state.
  for(int i(0); i<sys->N_hexamers; i++)
  {
    fprintf(Hfp, "%e\t%d\t%d\n", sys->tsim, hexamers[i].get_index(), hexamers[i].get_hex_S());
  }
  */
  //fclose( Hfp );
  ...
  /*
  // pSmpTVSnADP plot
  cout << (double)((pT+pD)-(pS))/(N_hexamers) << " " 
       << (double)hex_avr.CIIATP << " "                    
       << (double)(qATD + qITD + qAUT + qIUT + qASD + qISD + qAUS + qIUS)/N_hexamers << " " 
       << (double)(qAADPtoATP + qIADPtoATP)/(N_hexamers) << endl;  
  */
  /*
  // pTVSpS plot
  cout << (double)(pT+pD)/(N_hexamers) << " "  //s1                  
       << (double)(pS+pD)/(N_hexamers) << " "  //s2
       << (double)(qUT + qSD)/(N_hexamers) << " " //q(s1 -> s1+1)
       << (double)(qTU + qDS)/(N_hexamers) << " " //q(s1 -> s1-1)
       << (double)(qUS + qTD)/(N_hexamers) << " " //q(s2 -> s2+1) 
       << (double)(qSU + qDT)/(N_hexamers) << " " //q(s2 -> s2-1)               
       << (double) hexamers[0].get_CIIKaiA_bound() << endl;   
  */
  /*
  // phosVSnADP plot
  cout << (double)(pS+pD+pT)/(N_hexamers) << " " 
       << (double)hex_avr.CIIATP << " "                    
       << (double)(qAUS + qIUS + qATD + qITD + qAUT + qIUT + qASD + qISD)/N_hexamers << " " 
       << (double)(qAADPtoATP + qIADPtoATP)/(N_hexamers) << endl;  
  */     
  return hex_avr;  
}
