...
void Hexamer::register_state(int idx)
{
  state[0][idx] = 6-this->get_hex_CIATP();
  state[1][idx] = this->get_hex_pT();
  state[2][idx] = this->get_hex_pD();
  state[3][idx] = this->get_hex_pS();
  state[4][idx] = this->get_CIKaiA_bound();
  state[5][idx] = this->get_CIKaiB_bound();
  state[6][idx] = sys->tsim;
}
...
{
  double beta_energy( dGconfCIADP() + dGconfCIIADP() + dGconfKaiA() + dGconfKaiB() );
  //double beta_energy( dGconfCIADP() + dGconfKaiA() + dGconfKaiB() );  
...
/* Free energy difference between Active and Inactive state,
   dependent on ADP in CII domain. */
double Hexamer::dGconfCIIADP()
{
  int    nIIADPtot( 6 - this->get_hex_CIIATP() );
  double KATPADP(reaction_consts->KATPoKADP);
   
  double deltaF( log(KATPADP*KATPADP) ); 
   
  return deltaF *  nIIADPtot;
}
