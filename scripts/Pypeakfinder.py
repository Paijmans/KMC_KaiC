...
def period(xas):
	'''
	insert the name of the list that you want the mean, stdeviation and the difference between the first-neighbord in a list 
	'''
	xas=np.array(xas)
...
def datamax(filename):			
	'''
	insert the name of the file that you need max
	myfile.txt is the file to import
	'''
...
def atp_period(filename, dx, beg=0.05):						#insert the MASTER name of the list of file, with pass dx, that you want analyze,
	...
	maxx,minn = peakdetect(y,x_axis=x)
	
	
	x,m1,s1=period(maxx)
	
	x2,m2,s2=period(minn)
	
	b.append([m1,s1,inc])							# generate 2 lists where in each element you have the array of the max (min) the mean of that and the std and the increment(atp ratio)
...
def atp_period_plot(b,c):								# function that you inser the array of the max e min and he gives back the plot of the mean +- std
	...
	plt.errorbar(b[:,2],b[:,0],yerr=b[:,1],fmt='o', ecolor='g')
...
def autocorr0(x):
    ...
    result = result / result[maxcorr] 					#correlation
...
def atp_robustness(filename, dx, beg=0.05):						#insert the MASTER name of the list of file, with pass dx, that you want analyze,
	...
	for i in range(0,c):
		...
		buf= filename+ "%lf.txt" %(inc)
		
		data= np.loadtxt(buf)
	
		x,y= data[:,0], data[:,1]
		
		ff= autocorrelation(y)
		
		#ff=ff[::-1]
		
		par = curve_fit(funzia,x,ff)
		...
		b.append([n,L,inc])							# return a list where in each position you get the time period, the robustness parameter and the increment
...
def grafica(b):
	...
	plt.title("Robustness vs ATP ratio")
	plt.xlabel("ATP/ATP+ADP")
	plt.ylabel("n")
